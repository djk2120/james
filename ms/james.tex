...
\begin{abstract}
CLM5 introduces the Plant Hydraulic Stress (PHS) configuration, which is described and compared with the corresponding parameterization from CLM4.5.
...
The absolute value of SMS-OBS (Figure \ref{fig:t2}g) exceeds 1 mm/d in 67 of 414 days with available observations, as compared to just 23 with PHS.
...
PHS improves on SMS, with lower RMSE and higher correlation to transpiration observations, but yields a more pronounced high bias (Figure \ref{fig:t}b,d).
The wet season (February-March-April), especially, is prone to high biases in both models, where, in the observations, transpiration is reduced 32\% by TFE, as compared to modeled reductions of only 1.6 and 4\% for PHS and SMS, respectively.
...
Reproducing the effect of TFE (and the influence of soil moisture on leaf gas exchange) at Caxiuan\~a has precedent in the literature, and has not been captured well by the CLM (nor other DGVMs) \citep{restrepo2017,powell2013}.


\subsubsection{Soil Moisture}
The second source of observations for model evaluation is volumetric soil moisture. 
These data are valuable, because, in addition to water stress, the parameterization of root water uptake is significantly altered with PHS.
...
Under these conditions (excess rainfall), soil moisture is largely determined by the soil field capacity.
With water shortfalls, the root water uptake parameterizations drive the soil moisture dynamics, and the models diverge, 
with SMS consistently generating lower soil moisture values (than PHS) within the first meter of the soil column (Figure \ref{fig:sm2}, Supp Fig \ref{supp:sm2}).
...
Contrastingly, PHS achieves lower dry season soil moisture values under TFE.
...
Leaf water potential shows typical diurnal drop, reaching -1.65 MPa at midday (Figure \ref{fig:vwp}a).
...
Stem shade and leaf are all identical.
    
    Under TFE, model midday leaf water potential decreases to -2.31 MPa (Figure \ref{fig:vwp}b). 
    This change derives from a decrease in predawn root water potential (lower soil moisture) and an increase in the drop in root water potential between predawn and midday (due to reduced soil-to-root conductance).
    ...
    Despite reduced stem conductance, the pressure drop from root-to-stem actually decreases to -1.02 MPa, with transpiration reduced by 54\%.
    ...
    Midday leaf water potential features seasonal cycle, with lower values during the dry season.
    ...
    The parameter values used here may underestimate isohydricity (which would be reflected by minimal leaf water potential drop during drought) in response to TFE, showing a significant drop in leaf water potential of 0.66MPa (dry season, 2003), whereas observations showed no significant difference \citep{fisher2006}. 
...
\subsection{Stress dynamics}
    ...
    but PHS achieves more photosynthesis (Supp Fig \ref{supp:fpsn}), due to lower stress in the mornings and afternoons.
    ...
    PHS water stress reflects a requirement that plants avoid excessive xylem tension, which is associated with cavitation and mortality \citep{rowland2015}. 
...
Meanwhile PHS models significant stress, with $f_w$ as low as 0.5.
Despite abundant soil water, PHS still imposes stress, especially when VPD and downwelling solar radiation are high.
...
Based on transpiration observations (see Section \ref{sect:ttfe}), both configurations likely underestimate the drought effect during the wet season (discussed further in Section zqz).
SMS imposes more dry season stress, resulting in a 63\% reduction of GPP due to TFE, compared to 44\% with PHS.
Comparing wet season to dry season, and ambient conditions to TFE, PHS features less stress than SMS resulting from precipitation shortfalls.
The xylem tension stress of PHS imparts a negative feedback on photosynthesis and transpiration, described in Section zqz.

\subsection{Root water uptake}

In addition to updating water stress, PHS implements a new parameterization of root water uptake, consistent with hydraulic theory.
The parameterization of root water uptake affects the vertical distribution of soil water (Figure \ref{fig:sm}, Supp Fig \ref{supp:sm}), as
...
With PHS, RWU (within a given soil layer) is more sensitive to soil potential (Figure \ref{fig:rwu}), which prevents soil potential from getting much lower than -1 MPa. 

RWU is the product of hydraulic conductance ($k$) for water flow and the gradient ($\Delta\psi$) in water potential.
With PHS, reductions in RWU with drying are imposed by declining conductance (Supp Fig \ref{supp:cond}).
On the other hand, the hydraulic gradient tends to increase with drying (due to dynamic $\psi_{\text{root}}$), partially mitigating those reductions.
...
PHS results in higher sensitivity to soil potential, because conductance falls off by almost 3 orders of magnitude as soil potential declines from 0 to -1 MPa.
This derives mostly from the exponential dynamics of soil conductivity \citep{brooks1964}.
On the other hand, SMS $\Delta\psi$ decreases by less than half an order of magnitude over the same range.

While RWU within a given soil layer is more sensitive to soil potential with PHS (than with SMS), transpiration overall is less sensitive to shortfalls in precipitation
associated with dry season onset and TFE (Figure \ref{fig:t2}).
This is because, within PHS, there is more flexibility to compensate for dry layers by switching the water extraction for transpiration to moist layers.
As such, RWU is more sensitive to soil potential, but PHS compensates by spreading the transpiration sink over a larger vertical extent.
PHS is less sensitive to TFE, due to increases in RWU below 2 meters in depth (Figure \ref{fig:qdry}d).
...
\subsection{Stomatal conductance: Soil moisture stress vs. Xylem tension stress}
    ...
    with various relevent implementations described in the literature (see Section zqz).
    ...
    The result is significantly different stress dynamics on both diurnal and seasonal timescales (Figures \ref{fig:stress1},\ref{fig:gpp}).     
    
    Using leaf water potential (as in PHS) utilizes a physical justification, interpreting water stress as the result of increasing xylem tension, where vegetation will (according to the specific hydraulic parameter values) limit transpiration in order to avoid cavitation and embolism associated with overly negative water potentials, which has significant support in the literature \citep{novick2016a,sperry2017}.
    Soil water potential approaches (as in SMS) lack a straightforward physical basis, empirically relating stomatal conductance and/or photosynthetic parameters with soil potential (or soil moisture).
    ...
    Leaf water potential, on the other hand, is more easily measured in situ \citep{boyer1967}, and has been shown to correlate with remote sensing products \citep{momen2017}, which may offer better observational constraints for PHS.
    ...
    Therefore using $\psi_l$ as the driver of water stress preserves the relationship between stress and soil potential, but now also represents the contribution to water stress of xylem tension due to increasing demand for transpiration (reflected by increases in $\Delta\psi$).
    As such, PHS stress responds to changing soil moisture, but unlike SMS, also responds to VPD and downwelling solar radiation (Figure \ref{stress2}), which affect transpiration demand.
    ...
    With PHS, the relative contributions (to water stress) of changes in transpiration demand versus changes in soil potential (and likewise the strength of that negative feedback) can be varied by adjusting the hydraulic parameters. 
    establishing a flexible representation of vegetation water use dynamics that can be used to test hypotheses about drought resilience and water use strategies.
    
\subsection{Structural improvements in modeling root water uptake}

\subsubsection{Dynamic vegetation water potential}
\begin{itemize}
\item PHS introduces dynamic vegetation water potential
\item Observed in nature
\item Allows for dynamism in the gradient of water potential from soil-to-root, which controls RWU
\item Important for representing the range of water use strategies, and/or testing hypotheses regarding these.
\end{itemize}
\subsubsection{Hydraulic conductance, with response to drying}
\begin{itemize}
\item Likewise we have very solid science showing that hydraulic conductance declines with decreasing water potential in both plant vessels and soil substrate
\item You might ask, why does it matter that SMS doesn't capture this mode of variability in its (implied) conductance
\item After all, RWU does decline with soil drying!
\item Soil conductances feature an exponential relationship with soil potential, ranging three orders of magnitude over the range of soil potential observed in our simulations
\item Not really captured well by the linear loss of RWU exhibited in SMS from $\psi_o$ to $\psi_c$
\item As a result, PHS RWU is more sensitive to soil drying, which seems to ameliorate dry biases in soil moisture observed in SMS relative to observations.
\end{itemize}
\subsubsection{Compensatory RWU}
\begin{itemize}
\item Furthermore utilziing hydraulic approach enables compensatory root water uptake.
\item where, for example, as surface soil layers dry, tap roots can be used to harness reserves of soil water at depth to compensate for reduced RWU near the surface and maintain high levels of transpiration and photosynthesis.
\item This is observed in nature and its absence is a known shortcoming of the SMS-style transpiration sink
\item For example, PHS extracts 29\% of transpiration from beyond 2 meters depth under TFE (during the 2003 dry season) as compared to 13\% with SMS.
\item In PHS, as the surface soil layers dry out, conductance decreases rapidly, leading to reduced RWU.
\item In response, the vegetation `pulls' harder, lowering $\psi_{\text{root}}$, creating a larger gradient to the deeper soil layers, yielding increased RWU in the still-moist layers.
\item Due to constant vegetation potential ($\psi_c$) and no drying response to conductance, SMS lacks the flexibility to achieve this type of CRWU.
\end{itemize}
...
Because SMS does not resolve water potential through the vegetation substrate, the model instead uses a constant parameter, $\psi_c$, in its definition of $\Delta\psi$.
    ...
    PHS uses leaf water potential ($\psi_l$) to calculate the water stress factor, 
    ...
    Opting for xylem tension stress (PHS), in lieu of soil moisture stress (SMS), inserts an alternative physical interpretation of water stress and also yields different stress dynamics, 
    ...
    This negative feedback limits maximal transpiration and photosynthesis, while reducing variability.  
...
\section{Figures}
  ...
  \end{figure} 
    ...
    \begin{figure}[h]
     ...
     \caption{Vertical profile of soil water content (by volume) over time under 60\% throughfall exclusion, for
     (a) SMS, and 
     (b) PHS.
...
\section{Supplementary Figures}
  ...
  \begin{figure}[h]
     ...
     \caption{(a,b) Log of conductance versus soil potential for Soil Layer 5.
     (c,d) Hydraulic gradient ($\Delta\psi$) versus soil potential for Soil Layer 5.
     PHS conductance decreases by almost 3 orders of magnitude between 0 and -1MPa, 
