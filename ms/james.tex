...
Wet season OBS -32\%, PHS -1.6\%, SMS -4\%
Dry season OBS -60\%, PHS -48\%, SMS -62\%
...
SMS tends to achieve drier upper soil layers, whereas PHS spreads the drying over a larger vertical extent.
As described in Section zqz (Figure \ref{fig:sm2}), this is associated with a dry bias in the root-zone for SMS (within the dry season).
With PHS, RWU (within a given soil layer) is more sensitive to soil potential (Figure \ref{fig:rwu}), decreasing more sharply as soils dry as compared to SMS.

The dynamics in RWU are a combination of changes in the conductance ($k$) for water flow and the gradient ($\Delta\psi$) in water potential.
With PHS, reductions in RWU with drying are imposed by declining conductance (Supp Fig \ref{supp:cond}).
At the same time, the hydraulic gradient tends to increase with drying (due to dynamic $\psi_{\text{root}}$), partially mitigating those reductions.
...
While RWU within a given soil layer is more sensitive to soil potential with PHS (than with SMS), this does not mean that transpiration overall is more sensitive to shortfalls in precipitation.
As described in Section zqz (Figure \ref{fig:t2}), transpiration is less sensitive to dry season onset and to TFE than SMS.
This is because RWU within PHS has more flexibility to compensate for dry layers by increasing extraction from moist layers.
As such, RWU is more sensitive to soil potential, but PHS can spread the transpiration sink over a larger vertical extent.
...
\subsection{Can modeling vegetation water potential improve the CLM?}
    ...
    This improvement in model skill is not generalizable, but rather is specific to the site and experiment described herein, and model skill will need to be re-evaluated in a broader context.
    Instead, the value of opting for a single site (in lieu of global simulations) resides in the opportunity to perform more detailed analyses to elucidate the new model dynamics to complement this first description of PHS.
    

\subsection{Stomatal conductance: Soil moisture stress vs. Xylem tension stress}
    \label{sect:stress}
    
    The Medlyn stomatal conductance model requires a notion of water stress to attenuate stomatal conductance in order to reflect changes in water supply,
    with various implementations in the literature (see Section zqz).
    ...
    The result is significantly different stress dynamics on both diurnal and seasonal timescales (Figures \ref{fig:stress1},\ref{fig:gpp}, following from the contrasting relationships of stress with key forcing variables, including: soil moisture, vapor pressure deficit, and downwelling solar radiation (Figures \ref{fig:stress2}, Supp Fig \ref{supp:fsds}).
    Using leaf water potential (as in PHS) interprets water stress as the result of increasing xylem tension, where vegetation will (according to the specific hydraulic parameter values) limit transpiration to avoid cavitation and embolism associated with overly negative water potentials, which has significant support in the literature \citep{novick2016a,sperry2017}.
    There is no such physical interpretation for the SMS stress paradigm, but neither does it offer an easier route to parameter estimation, due to scarce observations of its input variable, root-fraction-weighted soil potential.
    
    Leaf water potential, on the other hand, is more easily measured in situ, and has been shown to correlate with remote sensing products, offering better observational constraints.
    ...
    Using $\psi_l$ to calculate stress preserves the relationship between stress and soil potential, but now also represents the contribution to water stress of xylem tension due to increasing demand for transpiration (which is proportional to $\Delta\psi$).
    As such, PHS stress responds to changing soil moisture, but unlike SMS, also responds to VPD and downwelling solar radiation (which affect transpiration demand).
    This imparts a diurnal pattern to PHS stress, with higher stress around midday, whereas with SMS, stress is relatively constant throughout the day.
    ...
    In our PHS simulations, this suppresses variability in GPP relative to SMS, and leads to underestimating the range of transpiration compared to sap flux observations. 
    With PHS, the relative contributions (to water stress) of changes in transpiration demand versus changes in soil potential (and likewise the strength of that negative feedback) can be varied by adjusting the hydraulic parameters, establishing a flexible representation of vegetation water use dynamics that can be used to test hypotheses about drought resilience and water use strategies.
