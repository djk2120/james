...
\section{Introduction}
 ...
 - droughts are increasing (I am not sure about this, I am not a fan of this statement as Sheffield et al showed the opposite, there is no change)
      . \citep{cook2015}
      . \citep{dai2013}
      . amazon dry season lengthening \citep{fu2013}
      . increase in freqeuncy of El Nino
     
There are uncertainies in soil msoiture projections but VPD is going to icnrease for sure
...
\label{sect:A}
    ...
    CLM5 photosynthesis, in its default configuration, is a two-big-leaf model, with a sunlit and shaded leaf per PFT per gridcell \citep{thornton2007, 
    ...
    $\beta_t$). This factor multiplies the rate of maximum carboxylation ($V_{\text{cmax}}$) as described in \citet{oleson2013} and first implemented in \citet{sellers1996a,sellers1996b}. Say here that there can be multiple choices but Lin et al. 2018 seems to suggest that not a good idea to multiply the whold conductance as not sensitive to soil moisture. ALso some field studies suggest Vcmax is not cahnging with drought, but model Vcmax implicitly accounts for mesophyl conductance changes.
...
\label{sect:gs}
    ...
    \end{equation}
    The model features two parameters $g_0$ ($\mu$mol / m2 / s) and $g_1$ (kPa$^{0.5}$). The $g_0$ parameter is
    minimum stomatal conductance, representing cuticular and peidermal losses (small). The $g_1$ parameter relates to the marginal water cost guiding the optimization of carbon assimilation.
    ...
    conductance indirectly via multiplication of $V_{\text{cmax}}$. Discuss the differnt possible choices such as 
...
1) isohydric behavior (deinfe isohydricity)
and
...
The current PHS parameterization of stem-to-leaf resistance is relatively simple compared to the literature \citep{franks2007}
and could be improved in future versions. However, in a model set to run at the global scale, there is always a tradeoff between added complexity and parameter reduction.
...
CLM uses a water stress factor (unitless, 0-1, stressed-unstressed), to attenuate $v_{\text{cmax}}$ with declining water status \citep{oleson2013}. 
Decreasing $v_{\text{cmax}}$, coupled via the stomatal conductance model and through the intercellular CO$_2$ concentration, 
results in decreases in photosynthesis, stomatal conductance, and transpiration.
...
However the dynamics of the two formulations are quite different, because with SMS the stress factor only responds to changes in soil potential,
...
As such, for PHS, we plot the monthly mean midday $f_w$ (see zqz).
...
Neither really gets TFE (even with soil parameter tinkering). 
PHS and SMS parameters are out-of-the-box.
Why can't we match TFE (PHS)?
Could we get closer by editing those parameters?
Difference in sensitivity to TFE is largely attributable to differences in root water uptake (see section zqz).
...
In Figure \ref{fig:vpd}c, TFE yields drier soils, showing the combined dependence of PHS $f_w$ on VPD and soil water.
...
Other groups take a hybrid approach, combining Medlyn stomatal conductance, with hydraulic constraints and/or 
...
We opt for a simplified form of this last approach, that seems to be consistent with field observations (Lin et al. 2018)
...
The vegetation water potential has two interfaces with the model.
As above, leaf water potential is used to affect drought stress for the photosynthesis/stomatal conductance routines.
Secondly, we use root water potential to calculate the soil transpiration sink term in Richard's equation.
PHS represents root water uptake via the Darcy framework where $q=k\Delta\psi$.
...
Looking at the conductance values associated with root water uptake from SMS vs. PHS serves to highlight the differences between paradigms.

Soil-to-root conductance ($k_{sr}$) is modeled in PHS and is used to calculate root water uptake (see zqz).
...
During FMA-2003, mean intra-day standard deviation of Layer 3 conductance is 4.6e-12 s$^{-1}$ with PHS
...
SMS comparable extraction from beyond 3 meters, but quite a bit more than PHS in the intermediate zone.
PHS surface extraction (Fig \ref{fig7}b, black/solid) responds to precip (Fig \ref{fig7}d),
SMS does not.

With tfe,
Neither PHS nor SMS, extracts much from between 1 and 4m.
