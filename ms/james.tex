...
%Water demand
    ...
    Water demand is calculated using the Medlyn stomatal conductance model (see Section zqz) modulated by the CLM water stress factor.
    As discussed earlier $f_w$ is based on leaf water potential \citep{klein2014} in PHS, where stress increases as leaf water potential becomes more negative.
    Because leaf water potential is modeled separately for sunlit and shaded leaves, $f_w$ takes on distinct sunlit and shaded values.
     ...
     neglecting non-linear components of the relationship between stress and transpiration (see next section).
...
We use a set of four simulations to assess the impact of the plant hydrodynamics model (PHS vs. SMS) on a through-fall experiment.
\begin{enumerate}
...
All four simulations use the same version of CLM5 (development version r270, https://github.com/ESCOMP/ctsm/releases/tag/clm4\textunderscore 5\textunderscore 18\textunderscore r270), which features a switch that can toggle between SMS and PHS configurations.
...
Selected parameter values concerning vegetation hydrodynamics are presented in Table 1. 
All other parameters use the default values associated with the r270 version of CLM5. 
Informed by parameter values reported in \cite{fisher2008}, we tuned soil hydraulic parameters and through-fall exclusion rates to get in range of observed soil moisture (Supp Fig \ref{supp:sm}).
An ensemble of 972 simulations was used to tune the parameters for the PHS configuration to fit sap flux observations (see Appendix zqz).
...
I plan to delete this section, after incorporating the main points into either the model description or previous discussion sections.
...
As such, the definition in the main text (Equation zqz), represents a linear simplification between stomatal conductance and transpiration.
Our formal definition eschews this simplification to improve model convergence.
...
The PHS iteration (see Figure) continues to convergence of $f_w$.
The numerics have proven to be stable in practice, but future versions may aim to better integrate PHS within the stomatal conductance scheme to avoid the linearity assumption within PHS.
...
All simulations used the same initial conditions, which were the result of the previous best simulation.
We evaluated the simulations based on the fit to sap flux observations.
We chose the ensemble member that maximized $R^2_{amb}+R^2_{tfe}-RMSE_{amb}-RMSE_{tfe}$ (Supp Fig \ref{supp:ens}).
