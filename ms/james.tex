...
Why is that attractive?
 - Model structure follows straightforward physics, darcy's law
 - While parameter estimation is challenging, trait information is available \citep{kattge2011,anderegg2015a} and valuable \citep{choat2012}
 ...
 - eschews bad empirical RWU
 - easy avenue to represent HR
 - can advance the modeled connection between allocation decisions and water access
 - evidence of complex regulation of water use in response to both soil and atmospheric drying
 - vegwp ends up being a really nice metric for water status
...
Utilizing a plant hydraulic framework extends the vegetation hydrodynamics parameterization, by explicitly modeling the flow of water through the vegetation substrate.
This allows for a better representation of differing water use strategies, and, very importantly, connects the notion of vegetation water status with atmospheric dryness.
...
Our goal:
  ...
  (1) is pretty appealing as the functional argument for several land surface processes, limits some of the discrepancies observed in terms of water stress repsonse
...
\label{sect:A}
    ...
    Some field studies suggest $V_{\text{cmax}}$ is not changing with drought, whereby model $V_{\text{cmax}}$ instead may implicitly account for mesophyll conductance changes.
