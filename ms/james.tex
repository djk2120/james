...
\label{sect:A}
    ...
    $\beta_t$). This factor multiplies the rate of maximum carboxylation ($V_{\text{cmax}}$) as described in \citet{oleson2013} and first implemented in \citet{sellers1996a,sellers1996b}.
     Say here that there can be multiple choices but \cite{lin2018} seems to suggest that not a good idea to multiply the whole conductance as not sensitive to soil moisture. 
    Also some field studies suggest Vcmax is not changing with drought, but model Vcmax implicitly accounts for mesophyll conductance changes.
