...
\subsection{Water stress and stomatal conductance}
    ...
    This is in addition to the VPD response of stomatal conductance.
    The Medlyn model features stomatal conductance proportional to VPD$^-0.5$, which serves to optimize carbon gain versus water losses \citep{medlyn2011}.
    Here we are trying to also impart xylem tension stress, where vegetation must limit transpiration to avoid cavitation and embolism.
    Support for this exists in the literature \citep{novick2016a,sperry2017}, but there is still no consensus on the best functional form \citep{zhou2013}. 
...
\subsection{Soil moisture and its influence on transpiration}
    ...
    Partly due to HR, vertical and temporal gradients in soil potential are significantly reduced with PHS (Figure \ref{fig:sm}).
    This is also related to the root water extraction gradient, which is significantly smaller with PHS (Supp Fig \ref{supp:cond2}).
...
\subsection{Caveats}
    ...
    [ need to incorporate this into model description sections]
