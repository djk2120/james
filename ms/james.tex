...
\begin{keypoints}
\item A simplified soil-plant-atmosphere continuum model based on hydraulic theory is implemented in CLM5.
...
As a result, significant uncertainty remains regarding how vegetation will respond to changes in hydroclimate within Earth System Models, feeding back onto the carbon cycle as vegetation mediates carbon uptake from continents \citep{dekauwe2017,friedlingstein2014}.


Plant water stress parameterizations are important in Earth System Models, as they define vegetation regulation of surface fluxes (photosynthesis, transpiration) to water fluctiations.
...
Drought stress parameterizations are used to define the response of stomtal conductance to vegetation water status that is used to attenuate transpiration, photosynthesis, and root water uptake with drying.
...
In this study, we developed a simplified plant hydraulic implementation within CLM5, called Plant Hydraulic Stress (PHS).
...
\label{sect:A}
    The CLM5 photosynthesis model is inherited from CLM4.5 as described in \citet{bonan2011}, \citet{thornton2007},
    ...
    The canopy fluxes module iterates to satisfy the leaf surface energy balance, while environmtal conditions are evolving.
...
\label{sect:gs}
    ...
    Those parameters are made plant function type dependent.
...
The four simulations are used to assess the impact of the plant hydrodynamics model (PHS vs. SMS) on a throughfall experiment (i.e., with either ambient or 60\% throughfall excluded), with all other model components and forcing shared.
...
Informed by \cite{fisher2008}, we tuned soil hydraulic parameters and throughfall exclusion to better capture the observational soil water dynamics (\cite{fisher2007} Figure 4), Supplementary Figure \ref{top3m}.
...
\label{sect:cond}
    ...
    In , we plot the time-series of conductance values from Soil Layer 3 (spanning from 6 to 12 centimeters below the surface) under ambient throughfall conditions during 2003, along with precipitation over the same period (PLUG IN CAPTION).
...
\label{sect:rwu}
    ...
    These fluxes are used as the soil transpiration sink terms within the Richards' equation (CITATION NEEDED) and, summed over the soil column, are equivalent to the transpiration flux.
    As described above (and in Section \ref{sect:btran}), PHS adopts a hydraulic framework for root water uptake. 
    . 
    PHS simulations (black color), feature more surface extraction than with SMS, especially under ambient throughfall conditions (Fig\ref{fig7}a), during the 2003 dry season.
    ...
    In these simulations, PHS is more sensitive to changes in soil water potential, with decreases in root water uptake in response to drying within a soil layer (Supp Fig \ref{supp:rwu}.
    ...
    PHS responds to rain.
    Opposite surface response to TFE
    PHS has more access to water at depth.
    ...
    Modeled surface extraction is especially high over the wet season under throughfall exclusion conditions, summing to 32.7 cm over 89 days.
    ...
    During the wet season surface extraction supplies transpiration, but also is used for hydraulic redistribution into lower soil layers. 
...
\subsection{Soil profile}
    ...
    Under 60\% throughfall exclusion, the change in soil water is -1.12 meters with PHS and -0.94 meters with SMS, relative to the ambient simulations (Supp Fig \ref{supp:buff}).
    ...
    The effect of $\psi_c$ extends to ambient throughfall conditions (Fig \ref{supp:sm}), but over a smaller spatial and temporal domain.
...
\section{Supplementary Figures}
  ...
  \end{figure}
    ...
    \begin{figure}[h]
     ...
     \caption{Time series of soil potential by soil layer, with SMS, under 60\% throughfall exclusion. 
