...
In this study, we develop a  plant hydraulic implementation within CLM5, based on hydraulic theory, which we refer to as the 'Plant Hydraulic Stress' (PHS) configuration. We analyse the dynamics of the new PHS model  using point simulations set within the Caxiuan\~a through-fall exclusion experiment \citep{fisher2006}.

Advancing the representation of the SPAC introduces the representation of vegetation water potential, (discretized into leaf, stem and root elements) into the CLM, as well as . an explicit representation of water supply, from the soil through the vegetation substrate.
...
Leaf water potential serves as an improved metric for water status, in lieu of soil water or soil matric potential, since it reflects vegetation sensitivity to both soil and atmospheric drying, while serving as a diagnostic for excessive xylem tension and cavitation risk. Modeling vegetation water potential and plant hydrodynamics also provides a framework for representing hydraulic redistribution \citep{lee2005} and connecting to remote sensing observations (e.g. Vegetation Opitcal Depth) \citep{konings206}. Further, root water potential is used to model gradient-based root water uptake based on Darcy;s law, providing the means to vary, for example, the mean depth of extraction with changing soil water conditions and replacing the previous empirical transpiration partitioning heuristic.  The new model can incorporate a range of water use strategies, improving the connection between allocation decisions and water availability.
...
\label{sect:rwu}
    ...
    SMS is limited by the extraction gradient, with root water uptake declining to zero as soil potential approaches -2.5 MPa (Supp Fig \ref{supp:rwu}).
