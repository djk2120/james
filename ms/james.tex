...
\subsection{Root water uptake: dynamics}
    ...
    Hydraulic conductance decreases approximately three orders of magnitude across the range of PHS soil potential (Supp Figure \ref{supp:cond}a,b).
    SMS implied conductance increases as the soils dry, until $\psi_{\text{soil}}$ reaches -2.5MPa (Supp Figure \ref{supp:cond}a,b).
    Root water uptake still decreases with SMS, but due to the decrease in $\Delta\psi$ (Supp Figure \ref{supp:cond2}c).
    Whereas PHS imparts a diurnal cycle to root water uptake through $\Delta\psi$, 
    $\psi_c$ (a constant parameter) defines the hydraulic gradient with SMS, requiring a diurnal cycle in the implied conductance (Supp Figure \ref{supp:cond2}).
...
\subsection{Root water uptake: profiles}
    ...
    However the two models feature distinct vertical profiles, where PHS removes less water from intermediate soil layers (0.5-1.5meters) (Figure \ref{fig:qdry}d).
    SMS can remove more water from these layers, due to the lower sensitivity of RWU to soil potential.
    ...
    After rain events, surface extraction renews, while at depth, root water fluxes reverse, depositing water.
    Both models decrease surface extraction with TFE (Figure \ref{fig:qdry}b), but PHS has a larger compensation from beyond 2m (Figure \ref{fig:qdry}c),
    ...
    with zero net root water uptake beyond 35.2 cm under ambient conditions (or beyond 9.6cm with TFE).
    ...
    sending that water deeper into the soil column.
...
\subsection{Soil moisture}
    ...
    Predawn root water potential averages to -0.14 MPa during SON under ambient conditions.
    With TFE, SMS dry season soil potential drops to -2.51 MPa and PHS to -0.5 MPa.
    ...
    It is not a coincidence that this value is equivalent to the SMS average under TFE in the last paragraph, 
    as $\psi_c$ defines the hydraulic gradient for root water extraction.
...
\subsection{Soil moisture effect on transpiration}    
    Model soil potential shows limited relationship to transpiration in the ambient simulations, 
    ...
    Sap flux observations under TFE shows a stronger relationship with PHS soil potential
     than with SMS soil potential (Supp Fig \ref{supp:cool}h,d).
    With SMS, the relationship with soil potential again seems to bias modeled transpiration with SMS (Fig \ref{fig:cool}b).
...
\subsection{The extensive influence of $\psi_c$}
    ...
    The non-linearity of root water uptake tends to make soil potential run to and then stick at $\psi_c$ (Figure \ref{fig:sm}).
    At the same time, $\psi_c$ serves as the sink potential relative to which the hydraulic gradient root water extraction is measured ($\Delta\psi = \psi_{\text{soil}}-\psi_c$).
    This defines the way root water uptake falls off with soil drying (Figure \ref{fig:rwu}), 
    which results in transpiration overall trending towards zero as average soil potential approaches -2.5 MPa (Supp Fig \ref{supp:cool}).
    
    SMS water stress factor is defined in part by $\psi_c$, the water potential with stomates fully closed.
    Meaning that how photosynthesis and transpiration fall off with shortfalls in precipitation is coupled to that parameter.
    Root water uptake is also based on $\psi_c$, where roughly speaking $q_i=k_i\left(\psi_{\text{soil},i}-\psi_c\right)$.
    Because $k_i$ does not decrease with soil potential in the SMS framework (Supp Fig \ref{supp:cond}), 
    water shortfalls tend to result in $\psi_{\text{soil},i}$ equivalent to $\psi_c$ (Figure \ref{fig:sm}).
    
    For the Broadleaf Evergreen Tropical PFT, $\psi_c$ equals -2.5 MPa \citep{oleson2013}, 
    leading to a strong dry bias in soil moisture in the first two meters of the soil column.
    
    At two meters and beyond, root fraction is too small to influence soil moisture very much.

\subsection{Structural improvements in modeling root water uptake}
    Conductance, based on brooks-corey, vulnerability, better response to soil moisture, allocation
    $\Delta\psi$, more dynamic, due to dynamic vegwp. eschews constraints
    
    The $k$ gives RWU more sensitivity to soil potential
    The $\Delta\psi$ (in tandem with declining $k$) gives the system more play, can allow you to switch to the lower layers as the surface dries out.
    Both constraints are visible in model functioning, HR precluded, $\psi_{\text{soil}}$ tends to $psi_c$.
...
\subsection{Hydraulic Redistribution (HR)}
    ...
    HR has been observed in the field \citep{oliveira2005} and has been highlighted as an important missing feature in CLM \citep{lee2005}.
    ...
    The predominate direction is downwards, sending water from rain events deeper into the soil column.
    ...
    Allowing HR into the top layer of the soil column (spanning 0 to 2 cm below ground) significantly degraded modeled soil evaporation (not shown). 
    To prevent HR from over-supplying evaporation, we set the hydraulic conductance to zero in that layer.
    Observations of HR are extremely difficult, and the degree to which HR actually occurs is unclear. 
    ...
    but it remains to be seen whether HR modeled in this way is a feature or a liability.
    PHS likely overestimates HR, lacking explicit representation of fine-root cavitation \citep{kotowska2015} and simplifying the root system architecture \citep{bouda2017}.
...
\section{Supplementary Figures}
  ...
  \begin{figure}[h]
     ...
     Beyond 2.5MPa, SMS implied conductance equals 0 (see Section zqz).}
     \label{supp:cond}
  \end{figure}
  \clearpage
