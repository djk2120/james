...
Capturing vegetation response is a high priority, especially within Amazonia.
...
On diurnal timescales drought parameterizations influence partitioning of latent versus sensible heat with effects on surface temperature.
On longer timescales vegetation water use strategies regulate the global carbon and water cycles, as well as subsurface hydrology.
...
SPAC adds complexity, but is in line with evidence of complex regulation of water use in response to both soil and atmospheric drying \citep{sperry2015}.
 - Model structure grounded in strong physical basis, darcy's law
 - Parameter estimation is challenging \citep{drake2017}, but trait information is available \citep{kattge2011,anderegg2015a} and valuable \citep{choat2012}
 - Likewise vegetation water status available at a scale that is relevant to model developement \citep{konings2016,grant2016} and valuable \citep{momen2017,konings2017b}
Allows you to model vegetation water potential, subject to a range of plant water use strategies
 - with vwp we can eschew bad empirical RWU and unphysical constraints 
 - vegwp ends up being a really nice metric for water status, because it reflects vegetation sensitivity to both soil and atmospheric drying
 - can be used to couple non-stomatal drought stress to the Medlyn stomatal conductance model
 - advances the modeled connection between allocation decisions and water availability

Our goal:
  - simplified plant hydraulic implementation
  - setting a framework to represent vegetation water potential, which is a very cool state variable
  - use vegwp to improve parameterization of drought stress and root water uptake

This study:
  - looking at a critical biome subject to experimental throughfall exclusion
  - demonstrating improved model dynamics achieved through modeling vegetation water potential
...
Simulations differ in only the plant hydrodynamics model (PHS vs. SMS) and throughfall forcing (Ambient vs. 60\% throughfall excluded).
Simulations are run offline, spanning from 2001 through 2003, utilizing the satellite phenology mode of CLM5.
All simulations start from the same initial conditions, which are the results of a 9-year spin-up repeating the PHS/Ambient simulation forcing three times.
For descriptions of site characteristics, forcing data, and observational sap flux, please see \cite{fisher2007}.
