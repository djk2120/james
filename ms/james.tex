...
\subsection{The promise of plant hydraulics}
    People are excited about plant hydraulics, especially if we can constrain parameter ranges and model complexity \citep{sperry2015,rogers2017}
    VWP is cool \citep{sperry2017,anderegg2017,mcdowell2018}
    Site-level models \citep{williams1996,sperry1998,bohrer2005}.
    Coupled to global models \citep{bonan2014,xu2016,christoffersen2016}.
    Here we implement plant hydraulics into CLM.
    ...
    while the added resistance from TFE is primarily from soil-to-root, consistent with previous results \citep{fisher2006}.
    No pressure drop stem-to-leaf (Figure \ref{fig:vwp}a,b), as we don't properly model leaf conductance \citep{franks2007}, but the functionality is there for those interested.
    
    The looming concern has been added complexity \citep{verhoef2014}, so we installed a simplified model.
    Evidence suggests complexity can be managed \citep{bartlett2016,mcdowell2018,powell2018}.
    Hydraulic models introduce new observational streams.
...
PHS uses leaf water potential to invoke water stress for modeling photosynthesis and transpiration.
Brief description of stomatal conductance theory. 
Point to the more recent studies.
Ours is a xylem tension stress.

Discuss results of stress

Discuss how stress influences GPP and transpiration.
