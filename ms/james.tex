...
\label{sect:btran}
    ...
    This yields $\beta_t$ analogs for the three terms of Equation \ref{phs:sink}.
    \begin{linenomath*}
    \begin{equation} \begin{aligned}
    ...
    With the $\beta_t$ formulation $k$ is not explicitly modeled, so instead we infer $k$, by dividing $q_i$ by the 
    $\beta_t$ notion of $\Delta\psi$ from Equation \ref{kb}.
    ...
    This is inconsistent with typical expressions, where $k$ is a function of some maximal conductance that is attenuated with drying.
