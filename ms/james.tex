...
\subsection{Root water uptake: dynamics}
    ...
    attributable to declining hydraulic conductance.
    ...
    Instead, root water uptake decreases due to loss of $\Delta\psi$ (Supp Figure \ref{supp:cond2}c).
    Whereas PHS imparts a diurnal cycle to root water uptake through $\Delta\psi$, 
    $\psi_c$ is constant with SMS, requiring a diurnal cycle in the implied conductance (Supp Figure \ref{supp:cond2}).
...
\subsection{Root water uptake: profiles}
    ...
    SMS can remove more water from these layers, due to lower sensitivity of RWU to soil potential.
    Likewise partitioning of root water uptake between near-surface and beyond is more sensitive to precipitation with PHS (Figure \ref{fig:qdry}a-c).
    ...
    with zero net root water uptake beyond 35.2 cm under ambient conditions (9.6cm with TFE).
    SMS extracts 49.8\% (amb) and 81.5\% (tfe) of total uptake from beyond those levels (Figure \ref{fig:qwet}). 
    PHS does extract farther into the soil profile, but in service of HR, 
    sending this water deeper into the soil column.
...
\subsection{Soil moisture}
    ...
    In lieu of straight average over the vertical profile, we use root-fraction weighted average for SMS.
    For PHS, we use predawn (5AM) root water potential, which more appropriately captures the effective soil potential (see section zqz).
    ...
    PHS better matches observations, especially in the top meter of the soil column, with RMSE lower by up to 55\% (Figure \ref{fig:sm2}, Supp Fig \ref{supp:sm2}).
    SMS root water uptake seems to pull too hard in the upper soil layers, with soil potential too low during the dry season.
    ...
    SMS root water uptake depends on $\psi_c$, with extraction set to zero when soil potential is drier than -2.5MPa.
    Beyond 1m, root fractions are smaller (Figure \ref{roots}), attenuating the SMS dry bias.
...
\subsection{Soil moisture effect on transpiration}    
    ...
    Sap flux observations under TFE shows a stronger relationship with PHS soil potential in the 
...
\section{Figures}
  ...
  \begin{figure}[h]
     ...
     Soil Layer 5 is shown, because it is near the surface (12 to 20 cm) and has a large root fraction (14\%).
