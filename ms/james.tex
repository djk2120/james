...
\subsubsection{SMS}
    ...
    Substituting for $w_i$ yields the SMS root water uptake equation as a function of the layer-$i$ soil potential ($\psi_i$).
    ...
    \\[1pt]
        \dfrac{T}{f_w} \dfrac{r_i}{\psi_{o}-\psi_{c}} \left(\psi_i-\psi_{c} \right)     & \text{if } \psi_{c} \le \psi_i \le \psi_{o} \\
    \\[1pt]
    \dfrac{T}{f_w} r_i    & \text{if } \psi_{c} \le \psi_i \le \psi_{o}
    \end{cases}
    \end{equation}
    \end{linenomath*}
    ...
    Fluxes from a given soil layer are driven by the gradient between soil potential ($\psi_i$) and the water potential in the root collar ($\psi_{\text{root}}$), 
...
%Hydraulic schematic and segmentation
  ...
  PHS solves for the set of SPAC vegetation water potential values ($\psi_{\text{root}}$, $\psi_{\text{stem}}$, $\psi_{\text{shade-leaf}}$, $\psi_{\text{sun-leaf}}$ ) that matches water supply (root water uptake) to water demand (transpiration), while maintaining continuity of water flow throughout the SPAC. Segmentation and other model design decisions followed a preference for a simplified implementation conforming to existing CLM model structure.
  
  At each node in the circuit diagram in Figure \ref{circuit} we model water potential, and, between nodes, we resolve the flux of water based on Darcy's law. The number of nodes is chosen as the strict minimum to allow allow for differences in segment parameterizations \citep{simonin2015, sperry2015}. Water uptake from the different soil layers is assumed to operate in parallel; a typical assumption justified by higher resistance in lateral versus central roots (e.g. Williams et al. 2001). Two resistors operate in series between each $\psi_{\text{soil}}$ and $\psi_{\text{root}}$, to capture the represent flows across the soil matrix and then through the root tissue \citep{williams1996}. 
