...
In Figure \ref{fig6}, we plot the time-series of conductance values from Soil Layer 3 (spans from 6 to 12 centimeters below the surface) under ambient throughfall conditions during 2003.

Well-watered values are very different, two orders of magnitude. 
Dynamics also very different, PHS conductance is relatively steady through the wet season (Fig \ref{fig6}a).
Through the dry season, the conductance generally declines with short resurgence episodes associated with rain events.
SMS is highly dynamic with more variability relative to the mean (Fig \ref{fig6}b).
...
This variability results from having SMS inferred conductance tethered to transpiration (see Section \ref{sect:btran}).
SMS features a clear diurnal cycle in the inferred Layer 3 conductance (Supp Fig \ref{supp:cond}). 
...
This is reinforced in Figure 7. With 60\% throughfall exclusion (Fig7a, dotted line), PHS conductance decreases, whereas with SMS, the daily mean conductance increases (Fig7b).

Why are the absolute values so different? When well-wetted
The SMS extraction gradient is measured relative to $\psi_c$ (a parameter defining soil potential with stomates fully closed), which equals -2.5MPa.
...
During TFE, PHS Layer 3 conductance actually gets basically just as low.
PHS-tfe Aug-2003 k=4.75e-11
SMS-amb Aug-2003 k= 3.89e-11

Which is a result of the orders of magnitude response of PHS k to changes in soil potential, as expected with soil conductances.
Inferred SMS k has no clear relationship with soil potential (Supp Fig \ref{supp:cond2}). 
...
HR has been observed in the field (cite Dawson, Oliveira), and has been highlighted as an important missing feature from CLM (cite jungeun).
...
But this is a new and significant functionality that we will have to grapple with. Should test it turned off.

SMS provides a useful heuristic, with the advantage of this type of heuristic is that you have guardrails.
But the consequent dynamics and drought response are underwhelming.
PHS offers improved model structure, which spans the expected dynamics.
PHS also offers a process connection between smp and leaf water potential and potentially VOD.
This is interesting, because it exposes smp to a new stream of observational data.
However, scale mismatch, parallel assumptions and parameter uncertainty may add up to hydraulics no more "physical" than a heuristic.
It's clear that we'll have more work to constrain the degrees of freedom in the PHS RWU vector, but on the other hand SMS seems plainly wrong.
The clear conclusion, which is hardly new, is that we need to find ways to constrain this process.
