...
\subsection{Water stress factor}
    ...
    The soil wilting factor ($w_i$) is a bounded linear function of soil matric potential ($\psi_i$).
...
\subsubsection{SMS}
    ...
    In the SMS configuration, a heuristic function is used to determine the root water uptake from a given soil layer ($q_i$).
    The function partitions the total transpiration ($T$) among the soil layers layers based on the product of the root fraction and the wilting factor, which must be normalized by $f_w$. 
    We then substitute for $w_i$ and rearrange to yield the SMS root water uptake equation (\ref{eq:btrwu}), subject to the restriction that $\psi_o \ge \psi_i \ge \psi_c$.
    ...
    \begin{equation}
    \label{eq:btrwu}
    q_i = \dfrac{T}{f_w} \dfrac{r_i}{\psi_{o}-\psi_{c}} \left(\psi_i-\psi_{c} \right)
    \end{equation}
    \end{linenomath*}
    
    In the Darcy framework, water fluxes are the product of hydraulic conductance ($k_i$) and hydraulic gradient ($\Delta\psi$).
    Where upon inspection of (\ref{eq:btrwu}), we can define hydraulic analogs resulting from the transpiration partitioning heuristic function, 
    ...
    Fluxes from a given soil layer are driven by the gradient between soil potential in that layer ($\psi_i$) and the water potential at the root collar ($\psi_{\text{root}}$), 
    ...
    accounting for the path across the soil matrix and through the xylem conduits (details in Section zqz).
    
    \begin{linenomath*}
    \begin{equation}
        \begin{aligned}
    q_i &= -k_{sr}  \left(\psi_{\text{root}}-\psi_{i}+\rho g z\right)
    \label{phs:sink}
    \end{aligned}
    \end{equation}
    \end{linenomath*}
