...
In addition to stress from soil moisture drought, vegetation is susceptible to increasing atmospheric demand \citep{restaino2016,novick2016b}leozqz.
...
Significant uncertainty remains regarding how vegetation will respond to changes in hydroclimate within Earth System Models, feeding back onto the carbon cycle as vegetation mediates carbon uptake \citep{dekauwe2017,friedlingstein2014}trugzqz.
...
2. Modeling plant hydrodynamics allows representing hydraulic redistribution \citep{lee2005}, as the flow repects Darcy's law and thus is alwyas directed down gradients of water potentials. 
...
To assess the new model formulation, we carried out site-level simulations at Caxiuan\~a National Forest in Brazil, which features a critical biome (terra-firme moist tropical evergreen forest) \citep{fisher2006}.  Starting in 2001, a plot at this site was subjected to an approximately 50\% percent precipitation through-fall exclusion. Due to the large drop in soil moisture at the precipitation exclusion site, significant vegetation water stress regulation of transpiration and photosynthesis was present at the site, prviding a natural experiment to test the model \citep{fisher2007}.

In this paper we therfore:
...
\label{sect:gs}
    The new CLM5 version implements the Medlyn stomatal conductance model, which reconciles the empirical and optimal approaches to 
    stomatal conductance, which aims at maximizing photosynthesis while minimizing transpiration \citep{medlyn2011}. 
    ...
    The Medlyn model, derived from stomatal optimization theory, predicts stomatal conductance to maximize assimilation relative to water transpiration costs ($A-\lambda E$), 
    but does not resolve concurrent limitations to stomatal conductance associated with drought conditions. To represent soil water stress, and its impact on leaf-gas exchange, land surface models typically include a `water stress factor'($f_w$, dimensionless, 0 to 1, formerly $\beta_t$). Uncertainty remains within the literature for how and where to apply water stress factors to photosynthesis and/or stomatal conductance. 
    ...
    \cite{lin2018} found that only $g_0$ was sensitive to soil moisture but not $g_1$ based on eddy-covariance observations.
    ...
    Other field studies, however, suggest that measured $V_{\text{cmax}}$ at the leaf level does not change with drought \citep{flexas2004}. On the other hand,  the modeled $V_{\text{cmax}}$ is a bulk measure of $V_{\text{cmax}}$, which implicitly account for mesophyll conductance changes \citep{flexas2004}, which has been shown to be water stress dependent.
    
    In PHS, we opt for a simplified form of this stress approach, which seems to be consistent with eddy-covariance observations and apply the stress function to the bulk modeled $V_{\text{cmax}}$ \citep{lin2018}. Prognostic water stress ($f_w$, ranging from 0-1) then attenuates stomatal conductance indirectly via multiplication of $V_{\text{cmax}}$. Water stress then lowers assimilation, and therfore stomatal conductance through a downregulation of photosynthesis ( CITE YOUR EQUATION ABOVE \citep{medlyn2011}).
    ...
    This reflects the concept of hydraulic safety, with vegetation avoiding excessive xylem tension associated with risk of cavitation. The previous stress formulation used instead the soil water potential, averaged across the root zone according to the root fractions.
    
    Utilizing leaf water potential instead of soil water potential for drought stress, introduces a new paradigm in the model as leaf water potential is regulated by supply of sap to the leaves and evaporative demand, regulated by stomatal regulation \citep{novick2016a}. As a result, low soil water (bottom-up stress) induces leaf moisture stress due to limited water supply but in addition high atmospheric VPD can also induce leaf moisture stress by propagating a drying into the xylem (top-down stress). This latter mechanism was absent from the previous water stress function (dependant on soil water potential only), by construction. Given the importance of rising VPD, it appears critical to include such mechanistic dependence of water stress though.
    
     I SUGGEST REMOVING THIS PARGRAPH APPEARS LIKE A TRAIN FO THOUGH, NOT THAT USEFULMuch recent work has focused on alternative constraints on plant hydrodynamics, especially in response to drying soils \citep{manzoni2013b,novick2016a,zhou2014}.
...
\subsection{Plant Hydraulic Stress (PHS)}
  The PHS model within CLM5 uses hydraulic Cohesion-Tension (Darcy's) law to model the flow of water through the SPAC, a simplified  electrical circuit analogy of which is presented in Figure \ref{circuit}. The hydraulic framework is used to diagnose leaf water potential and thus water stress associated with increasing xylem tension, which is then used to calculate root water uptake in each of (in this case) 20 vertically discretized soil layers using Darcy's law.
...
%Hydraulic schematic and segmentation
  ...
  At each node in the circuit diagram in Figure \ref{circuit} we model water potential, and, between nodes, we resolve the flux of water based on Darcy's law. The number of nodes is chosen as the strcit minimum to allow allow for differences in segment parameterizations \citep{simonin2015, sperry2015}. As in other versions of the CLM,  PHS uses vertically discretized soil layers  Water uptake from the different soil layers is assumed to operate in parallel; a typical assumption justified by higher resistance in lateral versus central roots (e.g. Williams et al. 2001). We further separate resistance through the soil matrix from the resistance through the root tissue \citep{williams1996}. 
  Specifics on the parameterization of conductance for each segment are provided in Appendix B.1.

%Water Supply
    \subsubsection{Water supply}
    \label{sect:supply}
    Water supply is modeled via Darcy's Law, where flow of water ($q$) is the product of the path hydraulic conductance ($k$) and the gradient in water plus gravitational potential $psi$.  Equation \ref{eq:darcy} represents the flow from a generic node 1 to node 2. 
    ...
    For simplicity, PHS does not represent plant tissue water storage (or capacitance, using the electrical circuit analogy).  Capacitance significantly complicates the water potential solution \citep{celia1990} and is challenging to parameterize \citep{bartlett2016}. However, buffering of water stress provided by tissue water storage could potentially be important especially on sub-daily timescales \citep{meinzer2009,epila2017}, whereby its inclusion may be warranted in future model generations.
...
%Water demand
    ...
    As discussed earlier, $f_w$ is based on leaf water potential \citep{klein2014}, and multiplies $V_{\text{cmax}}$, attenuating photosynthesis, and thus also stomatal conductance and transpiration.
    ...
    (Section \ref{sect:solution}).
     ...
     \end{eqnarray}
     \end{linenomath*}
    A nice feature of the new PHS formulation is that the parameters related to hydraulic loss of conductivity or stomatal regulation are traits that are routinely measured in the field and thus which can be used as constraints of the model. 
     
     In the olde formulation SMS, CLM featured two empirical water stress parameters, which are the soil matric potentials where stomata are either fully closed ($\theta_{wilt}$) or fully open  ($\theta_{crit}$)  (see Section \ref{sect:btran}).

%PHS solution
    \subsubsection{PHS solution}
    \label{sect:solution}
    
    PHS solves for the set of vegetation water potential values ($\psi$) that matches water supply (root water uptake) to water demand (transpiration), while satisfying continuity across the four water flow segments (soil-to-root, root-to-stem, stem-to-leaf, and leaves-to-transpiration). Numerically, at each time step, PHS computes the flux divergence $f$ (representing the mismatch of flow in and out of each segment for a given set of vegetation water potential values $\psi_i$, and iteratively updates $\psi$ until convergence i.e., $f\to0$.
...
\subsection{Root water uptake in SMS vs. PHS}
    ...
    For direct comparison between SMS and PHS, we recast (\ref{bt:4}) into the hydraulic framework: defining $T_{\text{max}}$, such that: $T = f_w T_{\text{max}}$ to replace $T$ in (\ref{bt:4}), and replacing $w_i$ in (\ref{bt:4}) with the formula from (\ref{bt:2}).
...
\subsection{Inconsistencies in SMS}
    ...
    \subsusection{Constant pulling potential}
    ...
    \subsusection{Conductance dynamics}
    ...
    \subsusection{No dependence on belowground carbon allocation}
    ...
    \subsusection{Lacks penalties for extraction from depth}
