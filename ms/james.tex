...
In Amazonia, which is the focus area of our experiments, studies suggest that the CLM (version 3.5) simultaneously underestimates the effect of experimental drought treatment \citep{powell2013} and overestimates dry-season reductions in GPP \citep{restrepo2017}.
...
\begin{enumerate}
\item Leaf water potential serves as a metric for water status compared to soil water or soil matric potential. As such, it reflects vegetation sensitivity to both soil and atmospheric drying, while serving as a diagnostic for excessive xylem tension and cavitation risk. 
\item Modeling plant hydrodynamics allows representing hydraulic redistribution \citep{lee2005}, as the flow respects Darcy's law and thus is always directed down gradients of water potentials. 
\item Further, root water potential can be used to predict gradient-based root water uptake based on Darcy's law, replacing the previous empirical transpiration partitioning heuristic. This provides the means to vary, for example, the mean depth of extraction with changing soil water conditions.
\item The new model can represent a range of water use strategies (i.e. isohydricity and anisohydricity), improving the connection between plant carbon allocation and water availability.
...
This study uses CLM5 to compare two parameterizations of root water uptake (Section \ref{sect:rwu}) and water stress (Section \ref{{sect:wsf}).  The first, which we refer to as Soil Moisture Stress (SMS), is the default parameterization used in CLM4.5.  The second is called Plant Hydraulic Stress (PHS); PHS is the default configuration of CLM5.
...
\label{sect:wsf}
    ...
    \subsubsection{CLM4.5 Stress Factor: SMS}
    \label{sect:fwsms}
...
\label{sect:smsrwu}
    ...
    Transpiration is partitioned among the soil layers based on the product of the root fraction and the wilting factor, which must be normalized by $f_w$. 
...
%Water demand
    ...
    \subsubsection{CLM5 Stress Factor: PHS}
    ...
    PHS implements an alternative to the SMS heuristic approach, using a mechanistic representation of root water uptake, following Darcy's Law.
...
\subsection{Water stress and stomatal conductance}
    ...
    Using leaf water potential tries to capture xylem tension stress, where vegetation must limit transpiration to avoid cavitation and embolism.
    This is in addition to the VPD response of the Medlyn stomatal conductance model,
    ...
    As such, stress now depends on transpiration demand, and, in turn, leaf-level VPD and radiation (Figure \ref{fig:stress2}, Supp Fig \ref{supp:fsds}).
    ...
    This might suggest model error external to the water stress parameterization.
...
\subsection{Hydraulic Redistribution (HR)}
    ...
    One challenge we faced was that HR seemed to oversupply the top layer of the soil column (spanning 0 to 2 cm below ground) and thus significantly degraded modeled soil evaporation (not shown). 
...
\subsection{Soil moisture dynamics}
    ...
    And due to the discontinuity of the root water uptake equation that occurs at $\psi_c$, soil water potential has a tendency to trend towards and then stick at $\psi_c$ during dry periods.
